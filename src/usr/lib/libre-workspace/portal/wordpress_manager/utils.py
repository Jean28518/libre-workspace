...
def create_wordpress_instance(name, domain, admin_password, admin_email, locale):
    ...
    os.sed("s/SED_PORT/" + random_port + "/g", f"{instance_dir}/docker-compose.yml")
    os.sed("s/SED_DB_PASSWORD/" + random_db_password + "/g", f"{instance_dir}/docker-compose.yml")
    os.sed("s/SED_ROOT_DB_PASSWORD/" + random_db_root_password + "/g", f"{instance_dir}/docker-compose.yml")
