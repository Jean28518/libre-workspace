...
echo "docker run -i -t -d -p 10923:80 --restart=unless-stopped --name onlyoffice -e JWT_ENABLED='true' -e JWT_SECRET='$ADMIN_PASSWORD' onlyoffice/documentserver --add-host cloud.$DOMAIN:$IP" > /root/onlyoffice/run.sh
